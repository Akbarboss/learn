...
2. `git push`.
3. Всё. У всех обновится при следующем запуске (нужен интернет).
...
Имена жёстко заданы в коде: **Akbar, Abror, Muhammadali, Zilola**. Свободного ввода
нет — только выбор из списка.
...
сразу видно всех, кто на нём занимался.
...
The default storage is local to the device. This project now uses the free
Firebase Realtime Database so the same learned-word counts appear on every
phone and in the APK. The Firebase rules are saved in [firebase-rules.json](firebase-rules.json)
and the public Web App settings are in [sync-config.js](sync-config.js).
